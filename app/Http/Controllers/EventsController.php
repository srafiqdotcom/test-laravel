...
class EventsController extends BaseController
{
    ...
    public function getEventsWithWorkshops() {
        try {
            ...
            $allEvents = $event->join($workShop->gettblename(), $workShop->gettblename() . '.event_id', '=', $event->gettblename() . '.id')->whereRaw('1')->groupby($event->gettblename() . '.id')->get();
            return $allEvents;
        } catch (\Exception $e) {
            log::critical("Exception Message ==>  ".$e->getMessage());
            log::critical("Exception Line ==>  ".$e->getLine());
            log::critical("Exception File ==>  ".__FILE__);
    ...
    public function getFutureEventsWithWorkshops() {
        try {
            $event = new Event();
            $workShop = new Workshop();
            $allevents = $event->join($workShop->gettblename(), $workShop->gettblename().'.event_id', '=', $event->gettblename().'.id')->whereRaw($workShop->gettblename().'.start >'.strtotime(date('Y-m-d')))->groupby($event->gettblename().'.id')->get();
            return $allevents;
        } catch (\Exception $e) {
            log::critical("Exception Message ==>  ".$e->getMessage());
            log::critical("Exception Line ==>  ".$e->getLine());
